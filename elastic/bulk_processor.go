...
type BulkIndexerConfig struct {
	...
	OnError      func(context.Context, []BulkIndexerItem, error) // Called for indexer errors.
	OnFlushStart func(context.Context, int) context.Context      // Called when the flush starts.
...
func (bi *bulkIndexer) Add(ctx context.Context, item BulkIndexerItem) error {
	...
	case <-ctx.Done():
		if bi.config.OnError != nil {
			bi.config.OnError(ctx, nil, ctx.Err())
		}
		return ctx.Err()
	case bi.queue <- item:
	}
...
func (w *worker) flush(ctx context.Context) error {
	if w.bi.config.OnFlushStart != nil {
		ctx = w.bi.config.OnFlushStart(ctx, len(w.items))
	}
	...
	if err != nil {
		...
		if w.bi.config.OnError != nil {
			w.bi.config.OnError(ctx, w.items, fmt.Errorf("flush: %s", err))
		}
